...
/** Published database name (see the `server:*` scripts in this repo). */
 ...
 * See this repo's README for local and deployed SpacetimeDB setup.
 */
...
export function normalizeSpacetimeClientUri(uri: string): string {
  ...
  try {
    ...
    throw new Error(
      `Invalid SpacetimeDB URI "${uri}". Use an absolute URL such as https://your-spacetimedb-service.onrender.com.`,
    );
  }
