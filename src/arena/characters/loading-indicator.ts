...
type LifecycleHost = {
	lifecycleCallbacks?: { update?: Array<(ctx: UpdateContext<any>) => void> };
};
...
function unregisterUpdate(
	host: LifecycleHost,
	callback: (ctx: UpdateContext<any>) => void,
): void {
	const list = host.lifecycleCallbacks?.update;
	if (!list) return;
...
export function createLoadingIndicator(
	...
	const onUpdate = ({ delta, me }: UpdateContext<any>) => {
		if (disposed) {
			unregisterUpdate(entity as unknown as LifecycleHost, onUpdate);
			return;
		}
		if (!ringMesh) return;
...
export function fadeInActor(actor: any): void {
	...
	let elapsed = 0;
	let done = false;
	const onUpdate = ({ delta }: UpdateContext<any>) => {
		if (done) return;
		elapsed += delta;
		...
		// Defer removal so we don't splice `lifecycleCallbacks.update`
		// while the entity is mid for-of iteration (which can skip the
		// next callback — e.g. local combat / cooldown input — for a frame).
		queueMicrotask(() => {
			unregisterUpdate(actor as LifecycleHost, onUpdate);
		});
	};
