...
const _flatScratch = new Vector3();
...
export function spawnLobProjectile(
	...
	const flat = _flatScratch.set(toward.x - from.x, 0, toward.z - from.z);
...
export function updateProjectiles(
	...
	for (let i = projectiles.length - 1; i >= 0; i -= 1) {
		...
		const hitR2 = p.hitRadius * p.hitRadius;
		let hit = false;
		for (const av of avatars.values()) {
			...
			// Planar reject before full 3D test.
