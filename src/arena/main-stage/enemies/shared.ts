...
const _avatarPosScratch = new Vector3();
const _moveDirScratch = new Vector3();
const _moveNextScratch = new Vector3();
 ...
 *
 * Writes into `target` (defaults to a shared scratch). Callers that need to
 * retain the value across further `avatarWorldPosition` calls must `.clone()`
 * or `.copy()` into their own Vector3.
 */
export function avatarWorldPosition(
	av: AvatarRecord,
	target: Vector3 = _avatarPosScratch,
): Vector3 | null {
	const tr = av.actor.body?.translation?.();
	if (tr) return target.set(tr.x, tr.y, tr.z);
	const g = av.actor.group;
	if (g) return target.set(g.position.x, g.position.y, g.position.z);
	return null;
}
 ...
 *
 * Reuses module scratch vectors; the returned Vector3 is stable until the
 * next `moveTowardXZ` call — copy it if you need to retain the value.
 */
export function moveTowardXZ(
	...
	const dir = _moveDirScratch.set(dest.x - current.x, 0, dest.z - current.z);
	const dist = dir.length();
	if (dist <= step || dist < 1e-6) {
		return _moveNextScratch.set(dest.x, groundedY(dest.x, dest.z), dest.z);
	}
	dir.multiplyScalar(step / dist);
	const nx = current.x + dir.x;
	const nz = current.z + dir.z;
	return _moveNextScratch.set(nx, groundedY(nx, nz), nz);
}
