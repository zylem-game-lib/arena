...
import type { TransformState } from '@zylem/game-lib/actions';
import { type UpdateContext, createCamera, createStage, type StageEntity } from '@zylem/game-lib/core';
import { createBox, createFog, createPlane, createText, type GameEntity } from '@zylem/game-lib/entity';
...
import {
	clearActiveParticleSystems,
	warmupParticleMaterials,
...
interface TransformableEntity {
	...
	playAnimation(options: { key: string; pauseAtEnd?: boolean }): void;
}
...
export function createArenaMainStage(): ArenaMainStageHandle {
	...
	// 24 subdivisions (~625 cells) keeps the bowl readable while cutting
	// mesh/heightfield cost vs the previous 40×40 grid.
	const GROUND_PLANE_SUBDIVISIONS = 24;
	...
	// Jumbotron geometry is kept for static collision mirroring only —
	// the screen is not added to the stage and we intentionally skip the
	// 1024×576 render-to-texture camera + orbit update (large GPU waste
	// while the feed is unused).
	...
	// the sky still reads as the background shader. Animated fog noise is
	// omitted — it was a steady per-frame shader tax for little visual gain.
	stage.add(createFog({
		type: 'exp2',
		color: '#c79ab0',
		density: 0.03,
	...
	// Compile particle textures + TSL pipelines during stage setup so the
	// first combat burst does not hitch on atlas / material build.
	warmupParticleMaterials(stage);
