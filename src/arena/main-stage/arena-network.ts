...
function applyNetworkAnimation(entity: PlayerEntity, t: EntityTransform): void {
	const prev = (entity as PlayerEntity & {
		__netAnimKey?: string;
		__netAnimPause?: boolean;
	});
	if (
		prev.__netAnimKey === t.animKey &&
		prev.__netAnimPause === t.animPauseAtEnd
	) {
		return;
	}
	prev.__netAnimKey = t.animKey;
...
export function bootstrapArenaNetwork(
	 ...
	 * and a known entity id — throttled + dirty-checked so idle standing
	 * does not spam ~60 WS messages/sec.
	 */
	function makeLocalTransformPusher(
		getLocalEntityId: () => bigint | null,
	): (payload: LocalTransformPayload) => void {
		const TRANSFORM_HZ = 20;
		const MIN_INTERVAL_MS = 1000 / TRANSFORM_HZ;
		const POS_EPS_SQ = 0.01 * 0.01;
		const ROT_EPS = 0.002;
		let lastSentAt = 0;
		let last: LocalTransformPayload | null = null;
		...
		return (payload) => {
			...
			const animChanged =
				!last ||
				last.animation.key !== payload.animation.key ||
				Boolean(last.animation.pauseAtEnd) !==
				Boolean(payload.animation.pauseAtEnd);
			if (!animChanged && now - lastSentAt < MIN_INTERVAL_MS) {
				return;
			}
			if (last && !animChanged) {
				const dx = payload.position.x - last.position.x;
				const dy = payload.position.y - last.position.y;
				const dz = payload.position.z - last.position.z;
				const posQuiet = dx * dx + dy * dy + dz * dz < POS_EPS_SQ;
				const rotQuiet =
					Math.abs(payload.rotation.x - last.rotation.x) < ROT_EPS &&
					Math.abs(payload.rotation.y - last.rotation.y) < ROT_EPS &&
					Math.abs(payload.rotation.z - last.rotation.z) < ROT_EPS &&
					Math.abs(payload.rotation.w - last.rotation.w) < ROT_EPS;
				if (posQuiet && rotQuiet) return;
			}
			...
			last = {
				...
				scale: { ...payload.scale },
				animation: {
					key: payload.animation.key,
					pauseAtEnd: payload.animation.pauseAtEnd,
				},
			};
